...
var omniGrid = new Class({
	...
	initialize: function(container, options){
		...
		if (this.options.sortOn) {
			this.defaultSortOn = this.options.sortOn;
		}
		if (this.options.sortBy) {
			this.defaultSortBy = this.options.sortBy;
		}
        ...
        this.filter_data = new Hash();

		//if (this.options.totalRow === true && this.options.pagination === false) {
		//this.options.totalRow = true;
		//} else {
		//	this.options.totalRow = false;
		//}

		//this.totals = new Hash();
	...
	reset: function(){
		...
		this.container.getElements('.th').each(function(el, i){
			...
			if (dataType) {
				...
				el.compare = function(a, b){
					...
					// @todo sort on raw data, not data in the table
					...
					if (dataType == 'number') {
						...
						var1 = parseFloat(var1.replace(/[^0-9.]/g, ''));
						var2 = parseFloat(var2.replace(/[^0-9.]/g, ''));
		...
		if (this.options.sortOn && !this.options.serverSort) {
			// Do sort on loaded data
			var sortIndex = null;
			this.options.columnModel.each(function(column, index){
				//console.log(column, index);
				if (column.header === this.options.sortOn) {
					//console.log(column, index);
					sortIndex = index;
					column.sort = this.options.sortBy;
				}
			}.bind(this));
			if (sortIndex !== null) {
				this.sort(sortIndex, this.options.sortBy);
			}
		}
	},
	...
	loadData: function (url)
	{
		...
		if (this.options.url) {
			...
			// Filters
			this.filter_data.each(function(value, key) {
			...
			// Load data from static variable passed into options object
	...
	setData: function(data, cm){
		...
		if (this.options.pagination) {
			...
			this.options.totals = data.totals;
	...
	overHeaderColumn: function(evt){
		...
		if (columnModel.sort === 'DESC') {
			evt.target.addClass('ASC_hover');
		} else if (columnModel.sort === 'ASC') {
			evt.target.addClass('DESC_hover');
		}
	},
	...
	outHeaderColumn: function(evt){
		...
		evt.target.removeClass('ASC_hover');
		evt.target.removeClass('DESC_hover');
	},
		...
		if (this.options.data) {
			...
			for (var r = 0; r < rowCount; r++) {
                ...
                for (var c = 0; c < columnCount; c++) {
                    ...
                    div.addClass('td');
                    //if (columnModel.autoFill === true) {
                        //div.setStyle('width', this.autoWidth);
                    //} else {
                    ...
                    /*	var img = new Element('img');
                        ...
                        div.innerHTML = columnModel.labelFunction(this.options.data[r], r, this);
                    } else {
                            ...
                            if (str == null || str.length == 0) {
                                ...
                                if (columnModel.type == 'html') {
                                    div.innerHTML = str;
                                } else {
                                    div.innerHTML = str.replace(/&/g,'&amp;').replace(/>/g,'&gt;').replace(/</g,'&lt;').replace(/"/g,'&quot;');
                                }
                            }
                    }
                } // for column
			...
			// Calculate Totals for Total Row if applicable
			if (this.options.totalRow === true) {
				if (this.options.pagination) {
					var totalBar = this.container.getElement('.totalDivPage');
					var fullTotalBar = this.container.getElement('.totalDiv');
				} else {
					var totalBar = this.container.getElement('.totalDiv');
					var fullTotalBar = null;
				}
				...
				if (fullTotalBar) {
					fullTotalBar.set('html', '');
				}

				for (var c = 0; c < columnCount; c++) {
					columnModel = this.options.columnModel[c];
					columnModel.total = 0;
					// Check if column model has a total calculation function
					//console.log(columnModel.totalCalcFunction);
					if (columnModel.noTotal === true) {
						continue;
					} else if (columnModel.totalCalcFunction != null) {
						columnModel.total = columnModel.totalCalcFunction(this.options.data);
					} else if (columnModel.dataType === 'number') {
						for (var r = 0; r < rowCount; r++) {
							columnModel.total += parseFloat(this.options.data[r][columnModel.dataIndex]);
						}
					}
				}
				...
				var li = new Element('li');
				ul.appendChild(li);
				totalBar.appendChild(ul);

				// Full total elements (when there is paging and totals)
				if (fullTotalBar) {
					var ful = new Element('ul');
					var fli = new Element('li');
					ful.appendChild(fli);
					fullTotalBar.appendChild(ful);
				}

				// Populate total row with data
				this.options.columnModel.each(function(column){
					var div = new Element('div').addClass('td').setStyle('width', column.width - (Browser.Engine.trident ? 6 : 6 ));
					if (column.noTotal !== true) {
						if (column.totalLabelFunction != null) {
							div.set('text', column.totalLabelFunction(column.total));
						} else {
							div.set('text', column.total);
						}
					} else {
						if (this.options.totalHeader == column.header) {
							if (totalBar.hasClass('totalDivPage')) {
								div.set('html', 'Page Total');
							} else {
								div.set('html', 'Total');
							}
						} else {
					...
					if (fullTotalBar) {
						// Use totals provided in JSON
						var fdiv = new Element('div').addClass('td').setStyle('width', column.width - (Browser.Engine.trident ? 6 : 6 ));
						if (column.noTotal !== true) {
							if (column.totalLabelFunction != null) {
								fdiv.set('text', column.totalLabelFunction(this.options.totals[column.dataIndex]));
							} else {
								fdiv.set('text', this.options.totals[column.dataIndex]);
							}
						} else {
							if (this.options.totalHeader == column.header) {
								fdiv.set('text', 'Total');
						...
						fli.appendChild(fdiv);
					}
				}.bind(this));
			}
		}

		//console.log(this.options.columnModel);
	},
	...
	draw: function() {
		...
		if (this.options.buttons) {
			...
			for (var i = 0; i < bt.length; i++) {
				...
				if (bt[i].onclick) {
					cBt.addEvent('click', bt[i].onclick.bindWithEvent(this, [bt[i].bclass, this]));
				}
		...
		for (var c = 0; c < columnCount; c++) {
			...
			div.store('dataIndex', columnModel.dataIndex);
			div.addClass('th');
			...
			if (columnModel.hidden) {
				...
				// Dynamically set the width of this column to fill any remaining space
				// Just storing in a variable now so we can come back and set the size once the other columns have been sized
				if (this.options.autoFill === columnModel.header) {
					this.autoFillColumn = div;
					this.autoFillColumnIndex = c;
					this.sumWidth -= columnModel.width;
					columnModel.autoFill = true;
				}
			}
		...
		// Determine autofill column if exists
		if (this.autoFillColumn) {
			this.autoWidth = hDivBox.getSize().x - (this.sumWidth) - (this.visibleColumns * 6);
			this.options.columnModel[this.autoFillColumnIndex].width = this.autoWidth;
			this.autoFillColumn.setStyle('width', this.autoWidth - 6);
			this.sumWidth = this.sumWidth + (this.autoWidth);
		}
		...
		if (this.options.height) {
			...
			// If pagination AND totals, make room for two bars (page total, and total total)
			if (this.options.totalRow && this.options.pagination) {
				var totalsToolbar = 26 + 26; // pagination toolbar height 25px + 1px bottom border
			} else if (this.options.totalRow) {
				// Just one bar if totals
				var totalsToolbar = 26;
			} else {
				var totalsToolbar = 0;
			}

			var bodyHeight = this.options.height-headerHeight-toolbarHeight-paginationToolbar-totalsToolbar-(Browser.Engine.trident ? 2 : 2 ); //+2 radi bordera
		...
		// Total row(s)
		if (this.options.totalRow === true) {
			if (this.options.pagination && !this.container.getElement('div.totalDivPage')) {
				var totalDivPage = new Element('div');
				totalDivPage.addClass('totalDivPage');
				totalDivPage.setStyle('width', width);
				totalDivPage.setStyle('height', 25);
				this.container.appendChild(totalDivPage);
			}

			if (!this.container.getElement('div.totalDiv')) {
				var totalDiv = new Element('div');
				totalDiv.addClass('totalDiv');
				totalDiv.setStyle('width', width);
				totalDiv.setStyle('height', 25);
		...
		if (this.options.pagination && !this.container.getElement('div.pDiv')) {
			...
			var h = '<div class="pGroup"><select id="rp" name="rp"><option ' + (this.options.perPage === 10 ? 'selected="selected" ' : '') + 'value="10">10</option><option ' + (this.options.perPage === 20 ? 'selected="selected" ' : '') + 'value="20">20</option><option ' + (this.options.perPage === 50 ? 'selected="selected" ' : '') + 'value="50">50</option><option ' + (this.options.perPage === 100 ? 'selected="selected" ' : '') + 'value="100">100</option><option ' + (this.options.perPage === 200 ? 'selected="selected" ' : '') + 'value="200">200</option></select></div>';
	...
	perPageChange: function(){
		//this.options.page = 1;
		//this.gotoPage(1);
		this.setPerPage(this.container.getElement('#rp').value)
		//this.options.perPage = ;
		//this.refresh();
	},
	...
	pageChange: function(){
		...
		//this.refresh();
		this.gotoPage(this.container.getElement('div.pDiv2 input').value);
	},

	setPage: function(p) {
		if (p > 0) {
			//console.log('maxpage', this.options.maxpage);
			if (this.options.maxpage > 0 && p > this.options.maxpage) {
				p = this.options.maxpage;
			}
		} else {
			p = 1;
		}
	...
	setPerPage: function(p, noRefresh){
		if (p > 0 && p != this.options.perPage) {
			// Set the rp drop down to value specified, if present
			this.options.page = 1;
			this.container.getElement('#rp').set('value', p);
	...
	sort: function(index, by){
		...
		// Clear existing column sort classes
		header.each(function(head){
			head.removeClass('ASC');
			head.removeClass('DESC');
			//console.log(head.get('class'));
		});

		if (!by || by.toUpperCase() === 'ASC') {
			by = 'ASC';
		} else {
			by = 'DESC';
		}
    ...
    filterBy: function (param, value) {
        this.setFilter(param, value);
        this.refresh();
    },

	setFilter: function(param, value) {
		this.filter_data.set(param, value);
		//this.filters.set(param, filter);
		//console.log(this.filters);
		if (this.filters[param]) {
			if (this.filters[param].get('value') !== value) {
				this.filters[param].set('value', value);
			}
		}
	},

	clearFilterBy: function(noRefresh){
		this.filters.each(function(filter, name){
			filter.set('value', '');
		});
		this.filter_data.empty();
	...
	attachFilter: function(name, filter) {
